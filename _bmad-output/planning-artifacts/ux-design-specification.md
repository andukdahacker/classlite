...
- **Center Admin (Secondary):** Responsible for the logistical backbone—rosters, schedules, and resource allocation—ensuring the center runs without friction.
...
4.  **Privacy & Compliance by Design:** Navigating Vietnam's Decree 13/2023/ND-CP requires clear UI for data subject rights (access, deletion) and transparent processing indicators without cluttering the "Lite" experience.
...
7.  **Content Architect AI Flow:** A "PDF-to-Interactive" conversion UI that allows teachers to transform static materials into editable digital exercises in seconds.
8.  **RBAC-Aware Workspace:** A UI that intelligently adjusts its surface area based on the user's role, hiding complex administrative tools from students and teachers while providing full CRUD access to Admins.
...
- **Teacher/Admin Context:** Desktop-first (optimized for "Grading Workbench" split-screen efficiency, "Master Calendar" drag-and-drop, and "Content Architect" creation).
...
- **Device Capabilities:**
  ...
  - **Persistent Client-Side Storage:** Robust offline-first auto-save for student submissions on unstable networks.
...
- **Smart Scheduling:** Dragging a class to a new slot in the **Master Calendar** automatically checks room/teacher conflicts and proposes resolution via a non-modal "Conflict Resolution" inline banner.
- **Role-Aware Visibility:** The UI automatically prunes actions. A student sees "Read" views for schedules, while an Admin sees "Drag-and-Drop" handles and "Add Session" buttons.
- **The "PDF-to-Live" Toggle:** One-click transformation of an uploaded document into an interactive exercise set.
...
1.  **Discovery (Onboarding):** _From "Overwhelmed" to "Relieved"._ Setup shouldn't be a 50-step wizard. It should be "Upload your logo, add one class, done."
2.  **Core Loop (Grading):** _From "Dread" to "Flow"._ Grading is usually a chore. The Split-Screen Workbench should create a "Flow State" where the teacher gets into a rhythm of approve-approve-edit-approve.
3.  **Completion (Publishing Grades):** _Satisfaction._ Hitting "Publish" shouldn't be scary. It should be a satisfying "Done" moment with visual confirmation that students have been notified.
4.  **Error State (Network Drop):** _Assurance._ Instead of a generic "Network Error" panic, the UI says "Offline Mode Active - We're saving your work locally. Keep going."

### Micro-Emotions

- **Trust:** Built by the "Evidence Anchors". When AI highlights a mistake, underlining the _why_ makes the user trust the tool.
...
2.  **Aggressive Gamification:** Duolingo can sometimes feel _too_ pushy. For B2B/Education, we want _encouragement_, not _annoyance_. Keep the "nagging" respectful.
...
graph TD
    ...
    E --> F[Auto-Save to Persistent Client-Side Storage]
...
Focuses on conflict resolution and center-wide visibility.

```mermaid
graph TD
    A[Start: Master Calendar] --> B[Drag Class Session to New Slot]
    B --> C{Conflict Detection}
    C -- No Conflict --> D[Update Database]
    C -- Conflict Found --> E[Show Inline Conflict Banner]
    E --> F{Resolution Type?}
    F -- Room Conflict --> G[Select Alternative Room from Available List]
    F -- Teacher Conflict --> H[Swap Teacher or Revert]
    G --> D
    H --> D
    D --> I[Push Notification to Class Participants]
```

### 5.5 Content Architect Flow ("AI-Assisted Creation")

Translates static content to interactive digital exercises.

```mermaid
graph TD
    A[Start: Exercise Builder] --> B{Creation Mode?}
    B -- Manual --> C[Rich Text Editor / MCQ Form]
    B -- AI-Generator --> D[Upload PDF/Word Doc]
    D --> E[AI Parsing Process]
    E --> F[Draft Questions Generated]
    F --> G[Refinement View: Preview & Edit]
    G --> H[Click 'Publish & Assign']
    C --> H
```
...
- **Performance Constraint:** Render ONLY the _active_ line (hovered). Rendering 50+ lines simultaneously will degrade scroll performance.
...
- **States:**
  - _Saving:_ Rotating "Feather" (Active background persistence).
  - _Saved:_ Green Checkmark (Data committed to **Persistent Client-Side Storage**).
  - _Offline:_ Amber "Cloud with Slash" icon + Sticky Banner ("Do Not Close Tab").
  - _Syncing:_ Pulsing Blue "Upload" icon (Background sync in progress).

#### 4. `MasterCalendar`

- **Purpose:** The logistical hub for center-wide scheduling.
- **Anatomy:** Full-screen grid with Draggable events. Overlay for Teacher/Room conflicts.
- **Constraint:** Must support multi-tenant filtering (view by Center, Room, or Teacher).

#### 5. `RBACWrapper`

- **Purpose:** Abstracting permission logic from UI components.
- **Implementation:** A higher-order component that takes `requiredPermission` (e.g., 'class:edit') and conditionally renders children or a "Disabled/Read-Only" state.

#### 6. `PrivacyCenter` (Vietnam Decree 13 Compliance)

- **Purpose:** Empowering users to manage their data subject rights.
- **Anatomy:** A dedicated section in "Settings" allowing users to:
  - **View Data:** Export all personal information in JSON/CSV.
  - **Edit/Correction:** Direct interface to update PII.
  - **Right to Erasure:** A multi-step confirmation flow to request/execute account and data deletion.

#### 7. `ContentModerator`

- **Purpose:** Screening AI-generated material for compliance with Decree 72/2013/ND-CP.
- **Mechanism:** An invisible background service that flags potential violations. In the UI, flagged items appear with a "Review Required" badge and a link to moderation guidelines before publishing.

### Implementation Roadmap

**Phase 1 (The Core Loop):** `GradingEditor` + `ConnectionLine` + `RBACWrapper`. Essential for the MVP "Grading Workbench".
**Phase 2 (Logistics & Content):** `MasterCalendar` + `ExerciseBuilder` (AI-Generator).
**Phase 3 (Compliance & Feedback):** `PrivacyCenter`, `ScoreDial` (Tactile input) + `CelebrationOverlay` (Confetti/Stamp animation).
**Phase 4 (Dashboard):** Complex charts for "Student Health".
...
- **Exception ("Anxiety Relief"):** For irreversible actions like "Publish Grades", we _do_ use a modal. Users want a moment of friction to confirm they are ready to broadcast scores.
...
- **Teacher/Owner (Desktop-First):** The complex "Grading Workbench" and "Dashboard" are optimized for **1440px+** screens.
  - _Tablet Adaptation:_ The "AI Sidebar" collapses into a drawer button to preserve the main reading area.
...
- **RBAC Audit:** Verify that "Student" role cannot access `/admin` or `/settings/center` URLs via manual entry.

## 9. Global Views & Settings

### 9.1 Profile View

- **Purpose:** Personal identity and activity tracking.
- **Components:** Avatar upload, contact info, and a "Personal Audit Log" showing recent login/activity history.

### 9.2 Settings View (Role-Specific)

The settings view dynamically adjusts based on the RBAC role:

- **Owner/Admin:**
  - **Center Branding:** Upload logo, set primary brand color (updates CSS variables).
  - **Methodology Configuration:** Upload "Golden Samples" for AI tuning.
  - **User Management:** Invite staff, toggle roles, deactivate accounts.
- **Teacher:**
  - **Grading Preferences:** Default grading SLA (e.g., 24h), custom feedback templates, and workbench layout (AI Sidebar left/right).
- **Student:**
  - **Accessibility:** High-contrast mode, text size adjustment, and sync priority (always sync vs. Wi-Fi only).

## 10. Domain Compliance & Reliability UX

### 10.1 Privacy by Design (Decree 13)

- **Transparent Processing:** Every time AI processes data (grading or exercise generation), a subtle "AI Processing" indicator is shown with a tooltip explaining what data is being used and why.
- **Data Rights UI:** The `PrivacyCenter` (see Component Strategy) provides a one-stop-shop for access, rectification, and erasure requests.

### 10.2 Content Moderation UI

- **Prohibited Content Alerts:** If AI detects content violating local regulations (Decree 72) in a student submission or teacher exercise, the item is locked with a "Compliance Review" overlay.
- **Moderator Workspace:** Admins see a dedicated queue for flagged content, allowing them to approve, redact, or delete.

### 10.3 Reliability & Sync States

- **Persistent Indicator:** Located in the global top bar.
  - **Green:** Connected & Synced.
  - **Amber:** Offline (Working on **Persistent Client-Side Storage**).
  - **Red:** Sync Error (Action required).
- **Background Sync Feedback:** When the network restores, a non-intrusive "Syncing..." spinner appears in the top bar, followed by a "Sync Complete" toast.
