...
interface ReadingExerciseSubmissionDetailsProps {
  exercise: Exercise;
  submission: Submission;
  isReviewing?: boolean;
}

function ReadingExerciseSubmissionDetails({
  exercise,
  submission,
  isReviewing = false,
}: ReadingExerciseSubmissionDetailsProps) {
  ...
  const { questionMap } = useMemo(() => {
    ...
    >();
    readingExercise.tasks?.forEach((task) => {
  ...
  const passageEditor = useEditor({
    extensions: [StarterKit],
    content: exercise.content as Content,
    editable: false,
    immediatelyRender: false,
  });
  ...
  const getQuestionNumber = (taskId: number, questionId: number) => {
    ...
    for (const task of readingExercise.tasks) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:h-[calc(100vh-var(--header-height)-2rem)] p-4">
      ...
      <div className="col-span-1 overflow-y-auto rounded-md border">
        <div className="p-6">
          {readingExercise.tasks?.map((task) => (
...
export { ReadingExerciseSubmissionDetails };
