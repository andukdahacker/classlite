...
export function CompletionDragAndDropSubmission({
  ...
  useEffect(() => {
    ...
    setOptions((prevOptions) =>
      (task.options ?? []).filter((opt) => !answeredValues.includes(opt)),
    );
  ...
  const handleDrop = (
    ...
    const getQuestionIndexByOrder = (order: number) =>
      task.questions.findIndex((q) => q.order === order);
    ...
    if (source === "options") {
      ...
      onAnswerChangeAction(taskIndex, questionIndex, item);
      ...
      if (isGap) {
        // Moving from gap to gap
        const targetQuestionIndex = getQuestionIndexByOrder(
          Number(targetOrder),
        );
        ...
        // Swap answers
        onAnswerChangeAction(taskIndex, targetQuestionIndex, item);
        onAnswerChangeAction(
          taskIndex,
          sourceQuestionIndex,
          displacedItem || null,
        ...
        // Moving from gap to options pool
        onAnswerChangeAction(taskIndex, sourceQuestionIndex, null);
  ...
  const contentEditor = useEditor(
    ...
    [currentTaskAnswers, isSubmitted], // Re-render editor when answers change
  );
