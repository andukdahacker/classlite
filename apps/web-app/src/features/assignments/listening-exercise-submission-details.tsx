...
function CompletionReview({
  ...
  questionMap: Map<string, { task: ListeningExerciseTask; question: any }>;
...
interface ListeningExerciseSubmissionDetailsProps {
  exercise: Exercise;
  submission: Submission;
  isReviewing?: boolean;
}

function ListeningExerciseSubmissionDetails({
  exercise,
  submission,
  isReviewing = false,
}: ListeningExerciseSubmissionDetailsProps) {
  ...
  const { questionMap } = useMemo(() => {
    const questions: { task: ListeningExerciseTask; question: any }[] = [];
    const questionMap = new Map<
      string,
      { task: ListeningExerciseTask; question: any }
    >();
    listeningExercise.tasks?.forEach((task) => {
  ...
  const { score, total } = useMemo(() => {
    const score = Object.values(markedCorrect).filter(Boolean).length;
    const total = questionMap.size;
    return { score, total };
  }, [markedCorrect, questionMap]);

  const getQuestionNumber = (taskId: number, questionId: number) => {
    let questionNumber = 0;
    for (const task of listeningExercise.tasks) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:h-[calc(100vh-var(--header-height)-2rem)] p-4">
      ...
      <div className="col-span-1 overflow-y-auto rounded-md border">
        <div className="p-6">
          {listeningExercise.tasks?.map((task) => (
            <div key={task.order} className="mb-8">
              ...
              {task.type === "Completion" ? (
                <CompletionReview
                  task={task as ListeningCompletionTask}
                      ...
                      {task.type === "Multiple choice" && (
                        <>
                          <div className="space-y-2 mb-2">
                            {(
                              question as ListeningMultipleChoiceQuestion
                            ).options.map(
                              (opt: ListeningMultipleChoiceQuestionOption) => (
                                <div
                                  key={opt.value}
      ...
      <div className="col-span-1">
        <Tabs defaultValue="material" className="h-full flex flex-col">
          <TabsList>
            <TabsTrigger value="material">Listening Material</TabsTrigger>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="rounded-md border">
                  <AppEditor editor={feedbackEditor} showMenu={isReviewing} />
                </div>
              </CardContent>
            </Card>
          </TabsContent>
...
export { ListeningExerciseSubmissionDetails };
